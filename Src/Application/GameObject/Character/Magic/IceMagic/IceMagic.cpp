...
namespace
{
	...
	constexpr float IceSplitSpreadAngle = DirectX::XMConvertToRadians(90.0f);
	...
	bool isSplitShot)									// 派生弾かどうか
{
	...
	m_splitCount = m_isSplitShot ? 0 : std::max(splitCount, IceMinSplitCount);
...
{
	return "Asset/Sounds/Magic/IceMagic/hit.wav";
}
...
bool IceMagic::CreateSplit(const std::shared_ptr<EnemyBase>& hitEnemy)
{
	...
	const float centerOffset = static_cast<float>(m_splitCount - 1) * 0.5f;

	for (int i = 0; i < m_splitCount; ++i)
	{
		// 弾数の中心を基準にして、左右へ均等に広げる.
		const float splitIndex = static_cast<float>(i) - centerOffset;
		const float angle = splitIndex * IceSplitSpreadAngle;
		const Math::Vector3 splitDir = RotateDirY(baseDir, angle);
		const Math::Vector3 startPos = m_pos + baseDir * IceSplitForwardOffset + sideDir * (splitIndex * IceSplitSideOffset);
